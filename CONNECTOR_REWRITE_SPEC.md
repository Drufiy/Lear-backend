...
| Single-source false-positive rate | On a fixture set with a known root cause, % of runs whose top hypothesis is wrong when diagnosing off one connector only | **0/3 (0%)** — measured 2026-09-06 on the 3 known k8s fixtures (config-missing, OOMKilled, missing-ConfigMap), all top hypotheses correct. Caveat: these are unambiguous single-signal cases where k8s events name the cause directly; the correlation gain is expected on *ambiguous multi-signal* incidents, which the M6 combined fixture (§7) provides — that harder set is the real comparison basis. **Status 2026-09-07:** still not measurable — the combined fixture's two legs (`broken-app`'s missing-config crash, `prash-test-synthetic-error-rate`'s synthetic metric) are semantically unrelated to each other, so there's no scenario where a single-source diagnosis is plausibly *wrong* for either to compare against. Closing this needs a purpose-built fixture where the same incident manifests in both systems with a genuinely ambiguous single-source read — not yet built. | Lower with 2-source correlation than single-source, on the same set |
