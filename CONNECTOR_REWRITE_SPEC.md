...
| Date | Who | Item | Note |
|---|---|---|---|
| 2026-09-07 | Aradhya (agent-assisted) | **Dogfooding bug fixes** (2 of the findings from Lear-on-Lear CI diagnosis) | **(1) Silent sub-diagnosis drop:** `diagnose_multi_failure` lost a whole CI job's diagnosis when the model returned a `files_changed` entry missing the required `explanation` field (Pydantic rejected the whole Diagnosis). Fixed in `diagnosis_agent.py`'s files_changed sanitizer — a missing/blank per-file `explanation` is now filled from `fix_description` (else a placeholder) so one missing sub-field can't sink the job. **(2) Diagnosing passing jobs → false positives:** the CI path diagnosed *every* job in the run's log ZIP, so it reported a passing job's non-blocking `ruff` warnings as the build failure. `diagnose_ci_run` now scopes to the run's actually-failed jobs via new `GitHubConnector.failed_job_names()` (GET /actions/runs/{id}/jobs); best-effort + graceful (jobs-API error or no name-match → falls back to all sections = prior behavior; explicit `failing_job_names` still wins). Residual: a suppressed-step warning *inside a genuinely-failed job* could still mislead — noted, not yet prompt-hardened. 6 new tests. Suite 676 passed / 9 skipped. Still open (chip): the deeper "why underperformed" prompt/log-capture investigation. |
